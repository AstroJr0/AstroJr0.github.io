...
(function () {
  ...
  function triggerAsteroids() {
    ...
    function dropNext() {
      ...
      ast.style.animation =
        "ce-asteroid-fall 0.45s cubic-bezier(0.4, 0, 1, 1) forwards";
      document.body.appendChild(ast);

      ast.addEventListener(
        "animationend",
        () => {
          ast.remove();
          burstRings(
  ...
  function triggerBlackHole(opts) {
    ...
    const doWH =
      opts.whitehole != null ? !!opts.whitehole : Math.random() < 0.01;
    const spd = 1.0;
    ...
    function drawBH(now) {
      ...
      try {
        ...
        const collFrac =
          elapsed > holdEnd
            ? Math.min(1, (elapsed - holdEnd) / T.collapseDur)
            : 0;
        const radius = maxR * growFrac;

        bx.clearRect(0, 0, bhc.width, bhc.height);

        const vg = bx.createRadialGradient(
          CX,
          CY,
          0,
          CX,
          CY,
          Math.hypot(bhc.width, bhc.height) * 0.7,
        );
        ...
        if (radius > 0.5) {
          for (let h = 3; h >= 1; h--) {
            ...
            const hg = bx.createRadialGradient(
              CX,
              CY,
              hr * 0.7,
              CX,
            ...
            bx.beginPath();
            bx.arc(CX, CY, hr * 1.35, 0, Math.PI * 2);
            bx.fillStyle = hg;
            bx.fill();
          }
        }
        ...
        if (radius > 0.5) {
          const cor = bx.createRadialGradient(
            CX,
            CY,
            radius * 0.82,
            CX,
          ...
          bx.beginPath();
          bx.arc(CX, CY, radius * 3.6, 0, Math.PI * 2);
          bx.fillStyle = cor;
          bx.fill();
    ...
    setTimeout(() => {
      swallowedEls.forEach((el, i) => {
        ...
        el.style.transform = `translate(${dx * 0.1}px,${dy * 0.1}px) scale(0.94)`;
  ...
  function resetUniverse(canvas, elements) {
    ...
    elements.forEach((el, i) => {
      setTimeout(() => {
        el.style.transition =
          "transform 0.75s cubic-bezier(0.2,0.9,0.2,1), opacity 0.65s ease";
        el.style.transform = "";
        el.style.opacity = "";
      }, i * 24);
    });
